    $(document).ready(function(){
	    ...
	    var grid=$("#grid-data").bootgrid({
		    ...
		    {
		    	if (row.state=='已结束') {
		    		return "<button class=\"btn btn-xs btn-default ajax-link command-run1\" data-row-id=\"" + row.process_instance_id + "\">查看详情</button>";
		    	} 
		    	if (row.state=='运行中') {
		    		return "<a class=\"btn btn-xs btn-info ajax-link\" target=\"_blank\" href=\"traceprocess/" + row.process_instance_id + "\">查看进度</a>&nbsp;&nbsp;"
		    		+"<button class=\"btn btn-xs btn-default ajax-link command-run1\" data-row-id=\"" + row.process_instance_id + "\">查看详情</button>";
		    	}
		    }
	    	}
